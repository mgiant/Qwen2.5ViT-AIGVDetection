...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=False, **kwargs):
    ...
    def forward(self, pixel_values, **kwargs):
        ...
        if is_video:
            ...
            pred = pred.mean(1)
        prob = pred[:, 0].sigmoid()
        pred_dict = {'cls': pred, 'prob': prob, 'feat': None}
        return pred_dict
...
def resnet50_npr(pretrained=False, **kwargs):
    ...
    if pretrained:
        # model_state = torch.load(model_zoo.load_url())
        model_state = torch.load(model_urls['resnet50'])
...
if __name__ == '__main__':
    ...
    model = resnet50_npr()
    model.eval()
    input = torch.randn(2, 8, 3, 448, 448)
    output = model(input)
    print(output.size())
    output = model(input, return_feature=True)
    print(output.size())
